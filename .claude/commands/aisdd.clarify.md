---
description: 需求澄清。AI 主动提问，明确需求的方方面面（范围、行为、验收、角色、边界等），将答案整合回 spec；不讨论技术方案，技术选型与实现细节在 plan / epicdesign 阶段讨论。
handoffs:
- label: 制定技术方案
...
**定位与使用场景**：
- **clarify**：用于 **首次交互式需求澄清**，在 `/aisdd.featureplan` **之前** 执行
...
- **不讨论技术方案**：不得提出技术选型、架构设计、实现方式、技术栈、存储/框架/接口设计等方案类问题；上述内容在 `/aisdd.featureplan` 与 `/aisdd.epicdesign` 阶段讨论
- **可涉及的需求侧约束**：若产品/业务已明确「必须满足的合规要求、部署环境、上项范围」等，可澄清并记录；不得引导用户做技术实现层面的选择

**后续需求变更**：若已完成 clarify，后续需求有变化，可直接说明要更新的范围（如 FR/NFR、验收标准、某章节），由 AI 更新 spec 并视需同步 plan/设计；建议在文档变更记录中留痕。

注意：该澄清流程需在调用 `/aisdd.featureplan` **之前** 完成。若用户明确表示跳过澄清（如探索性原型开发），可继续执行，但必须提示后续返工风险会增加。
...
2. 加载当前规格说明文件。使用以下分类体系进行结构化的模糊点与覆盖度扫描。对每个分类标记状态：清晰（Clear）/ 部分明确（Partial）/ 缺失（Missing）。生成用于优先级排序的内部覆盖度映射表（除非无需提出任何问题，否则不输出原始映射表）。
   ...
   对于每个状态为“部分明确”或“缺失”的分类，新增候选澄清问题（除非满足以下条件）：
    - 澄清内容不会实质性改变实现或验证策略
    - 该问题属于**技术方案/选型**范畴（此类问题排除，留待 plan / epicdesign 阶段）
...
3. （内部）生成优先级排序的候选澄清问题队列（最多5个）。**禁止一次性输出所有问题**。需遵循以下约束：
    ...
    - 每个问题的答案类型需为以下之一：
        - 简短选择题（2-5个互斥的明确选项）
        - 单字/短句回答（明确限制：“答案≤5个字”）。
    - 仅包含**需求侧**问题：答案会对范围、行为、验收、角色、数据含义、边界场景、可测试性、合规/业务约束等产生实质性影响；**排除**技术选型、架构设计、实现方式类问题（留待 plan / epicdesign）。
...
5. 每接收一个有效答案后，立即整合至规格说明（增量更新方式）：
    - **写入前纯净度自检（强制，每次整合前执行）**：按 `aisdd.featurespec.md` §「spec / 技术细节边界守护」8 类清单扫描即将整合的答案文本及改写后的 FR/NFR/AC/场景条目；命中污染时**不得整合写入**，按 block_ask 向用户展示原文、命中类别与建议归属，请用户四选一：（a）改写为业务语言后留在 spec.md（推荐）；（b）移出 spec，记入 plan/epic-design 待办；（c）确认保留并说明理由；（d）拆分业务部分留 spec、技术部分移下游。详见 `.cursor/rules/aisdd-document-boundaries.mdc`。
...
8. 完成报告（提问循环结束或提前终止后）：
    ...
    - 若存在未解决或已推迟的分类，建议是否继续执行 `/aisdd.featureplan`，或在方案制定后再次运行 `/aisdd.clarify`。
    - 建议的下一步指令。
