...
- 技术栈偏好或限制
- 质量标准要求（如性能、可靠性等）
- 是否依赖外部团队（后端/其他端）联调、是否有测试用例可对照自检

如果信息不足，优先询问：
1. 这个功能的核心价值是什么？
2. 有哪些必须实现的关键功能点？
3. 有没有特殊的技术要求或限制？
4. 是否需要与外部团队联调？联调方和接口是否已约定？
5. 是否有测试用例或验收标准用于开发完成后自检？
...
#### 6. 外部依赖与联调
- **无/简单 (1 分)**: 无外部团队依赖，或仅调用已稳定接口
- **中等 (2-3 分)**: 需与 1 个外部团队联调，接口约定清晰，排期可协调
- **复杂 (4-5 分)**: 多团队/多系统联调，接口未定或易变，依赖对方排期与环境
...
### 5. 第三方集成与外部联调 (X 小时)
- [第三方库] 集成
- 与外部团队联调：接口对齐、Mock/联调环境、联调验收（无依赖时可填 0）

### 6. 测试（含自测）(X 小时)
- 单元测试 / UI 测试 / 集成测试
- 对照测试用例自测（自检）：逐条执行、记录问题
...
| 复杂度 | UI/屏幕 | 数据模型 | 算法实现 | 第三方集成与联调 | 业务流程 |
|--------|---------|----------|----------|------------------|----------|
| 简单 (1-2) | 2-4h | 1-2h | 1-3h | 1-6h | 2-4h |
| 中等 (3) | 4-8h | 2-4h | 3-8h | 6-20h | 4-8h |
| 复杂 (4-5) | 8-16h | 4-8h | 8-20h | 12-40h | 8-16h |

**交付阶段基准（在开发+测试之外单独估算）：**

| 阶段 | 计算方式 | 说明 |
|------|----------|------|
| 自检 BUG 修复 | 核心开发工时 × 0.2~0.4 | 修完自测问题才能提测，按历史缺陷率调整 |
| 提测后 BUG 修复 | 核心开发工时 × 0.25~0.5 | 多轮回归直至稳定，新功能/改动大取上限 |

#### 计算公式

```
核心开发工时 = Σ(各维度复杂度对应的基准工时，含第三方集成与联调)
测试工时 = 核心开发工时 × 0.3（含对照测试用例自测）
优化调试 = 核心开发工时 × 0.2
开发总工时 = 核心开发工时 + 测试工时 + 优化调试

自检 BUG 修复 = 核心开发工时 × 0.2~0.4
提测后 BUG 修复 = 核心开发工时 × 0.25~0.5

总工时 = 开发总工时 + 自检 BUG 修复 + 提测后 BUG 修复
...
- **阶段 1 开发完成**: [里程碑描述] - 预计 X 天
- **阶段 2 联调通过**（如有）: [联调验收] - 预计 X 天
- **阶段 3 自测通过**: 自测 BUG 修复完毕、达到提测标准 - 预计 X 天
- **阶段 4 提测**: 提交测试 - 预计 X 天
- **阶段 5 版本稳定**: 测试 BUG 修复完毕、回归通过 - 预计 X 天
...
### 评估误区
- ❌ 忽略测试和调试时间
- ❌ 忽略与外部团队的联调时间（接口对接、环境、排期）
- ❌ 忽略测试阶段中的自测时间（自测=自检，已含在“测试”工时内）
- ❌ 忽略自检 BUG 修复时间（未预留“修完再提测”的缓冲）
- ❌ 忽略提测后多轮修 BUG 直至稳定的时间
- ❌ 低估第三方库集成的复杂度
- ❌ 没有考虑需求变更的可能性
- ❌ 忽略代码质量和重构的时间
