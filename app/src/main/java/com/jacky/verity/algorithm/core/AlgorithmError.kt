...
/**
 * 算法引擎错误类型（继承自Exception，以支持Result.failure()）
 */
sealed class AlgorithmError(
    ...
    // 计算错误
    class CalculationError(cause: String) : AlgorithmError(
    ...
    // 数据错误
    class DataError(cause: String) : AlgorithmError(
    ...
    // 输入无效
    class InvalidInputError(field: String, reason: String) : AlgorithmError(
    ...
    // 未知错误
    class Unknown(cause: Throwable?) : AlgorithmError(
