...
sequenceDiagram
    ...
    HW->>SF: Vsync 信号（~16.67ms/帧）
    SF->>CH: 分发 Vsync（通过 EventThread）
    CH->>MT: doFrame() 回调
    MT->>MT: 处理输入事件 (Input)
    MT->>MT: 执行动画 (Animation)
    MT->>MT: View Tree 遍历 (Traversal)<br/>measure → layout → draw
    MT->>RT: syncFrameState（同步 DisplayList）
    RT->>RT: 执行 OpenGL ES/Vulkan 命令
    RT->>BQ: dequeueBuffer → 渲染 → queueBuffer
    BQ->>SF: onFrameAvailable 通知
    SF->>SF: 合成所有 Layer（HWC 或 GPU）
    SF->>HW: 提交到显示控制器
    HW->>HW: 下一个 Vsync 显示到屏幕
```
