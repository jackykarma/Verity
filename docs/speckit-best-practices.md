...
```bash
# 默认执行 Lite 阶段（基础框架 + Story 拆分）
/speckit.plan

# 执行 Standard 阶段（需先完成 Lite）
/speckit.plan --phase=standard

# 执行 Deep 阶段（需先完成 Standard）
/speckit.plan --phase=deep

# 一次性执行所有阶段
/speckit.plan --phase=all
```
...
Plan Level（Lite/Standard/Deep）标识**需要达到的最终深度**，分阶段输出是**逐步产出的执行方式**：

| Plan Level | 推荐执行方式 |
|------------|-------------|
| **Lite** | 只执行 Lite 阶段即可 |
| **Standard** | 先 Lite，再 Standard |
| **Deep** | 先 Lite，再 Standard，最后 Deep |

**建议工作流**：
1. 先执行 `/speckit.plan`（Lite 阶段），产出基础方案
2. 基础方案评审通过后，按需执行 `--phase=standard` 补充风险/评估
3. 若为高风险功能，再执行 `--phase=deep` 补充 Story 级详细设计
...
```
/speckit.plan-update "范围：A2 架构"
/speckit.plan-update "范围：A3.2 全景类图"
/speckit.plan-update "范围：Story Breakdown"
```
...
Plan 支持**分阶段执行**，每个阶段逐步深入：

- **Lite（默认给小需求）**：
  - 执行：`/speckit.plan`（默认 Lite 阶段）
  - 产出：A0-A2、A3.1-A3.2、Plan-B、Story Breakdown
  - 流程：`spec → plan(Lite) → tasks → implement`
- **Standard（默认给大多数需求）**：
  - 执行：先 `/speckit.plan`，再 `/speckit.plan --phase=standard`
  - 产出：在 Lite 基础上追加 A3.3、A4-A11（风险/评估）
  - 流程：`spec → plan(Lite→Standard) → tasks → implement`
- **Deep（少数高风险）**：
  - 执行：Lite → Standard → `/speckit.plan --phase=deep`
  - 产出：追加 Story Detailed Design（L2 二层详细设计）
  - 流程：`spec → plan(Lite→Standard→Deep) → tasks → implement`

> 建议规则：**默认只跑 Lite**，评审通过后按需追加 Standard/Deep。Deep 必须有触发条件（可写入 CR 或 Plan 变更记录）。
