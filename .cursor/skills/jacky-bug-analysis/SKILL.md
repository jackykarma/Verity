...
使用 `jacky-bug-classifier` skill 对 bug 进行分类:

```markdown
请使用 jacky-bug-classifier skill 分析以下 bug:
[粘贴收集到的 bug 信息]
```
...
|---------|-----------|---------|
| Crash / ANR | `jacky-crash-anr-specialist` | 应用崩溃、无响应、强制关闭 |
| Performance | `jacky-performance-specialist` | 卡顿、慢启动、帧率下降 |
| Memory | `jacky-memory-specialist` | 内存泄漏、OOM、内存占用过高 |
| Concurrency | `jacky-concurrency-specialist` | 死锁、竞态条件、线程问题 |
| UI/Rendering | `jacky-ui-rendering-specialist` | 界面错乱、渲染问题、布局异常 |
| Logic/State | `jacky-logic-state-specialist` | 业务逻辑错误、状态不一致 |
...
助手: 让我分析这个崩溃问题。
1. 先收集 logcat 日志
2. 使用 jacky-bug-classifier 确认是崩溃类型
3. 调用 jacky-crash-anr-specialist 进行深度分析
4. 生成修复方案
```
...
助手: 让我分析这个性能问题。
1. 收集性能指标 (帧率、CPU、内存)
2. 使用 jacky-bug-classifier 确认是性能类型
3. 调用 jacky-performance-specialist 进行分析
4. 提供优化建议
```
