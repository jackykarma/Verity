...
2. **前置条件检查（所有 Feature spec 已就绪）**：遍历 `EPIC_DIR/features/` 下每个**子目录**，若某子目录存在且其中**无 `spec.md`**，则**终止**并提示：「须在**所有** Feature 的 spec 输出后再运行 /aisdd.epicuidesign。以下 Feature 目录尚未具备 spec：\[列出缺 spec 的目录名\]。请对缺 spec 的 Feature 运行 /aisdd.feature 或 /aisdd.specify，待全部完成后再运行本命令。」
...
6. **完成报告**：输出 ux-design.md 路径（EPIC 根）、design 路径（若已创建），并提示下一步：若尚未做 EPIC 技术规约可运行 `/aisdd.epicplan "EPIC-xxx"`；对各 Feature 设置 `SPECIFY_FEATURE` 后运行 `/aisdd.plan`。
