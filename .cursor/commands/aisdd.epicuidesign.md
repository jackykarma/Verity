...
> 目的：AI 用绘图展现对设计稿 UI 结构的理解，让开发者直观 review AI 是否正确理解设计意图，减少下游返工。输入质量 D 级（兜底模式）或纯交互侧重时可跳过。

完成视觉规范章节提取后，可为以下内容生成理解验证图（使用图像生成工具）：

1. **关键界面布局结构图**：对界面清单中标记为核心的界面生成线框级布局草图（区域划分、关键组件位置与相对大小、组件类型标注）
2. **状态切换示意图**（若涉及多状态界面）：在一张图中展示 Loading / 空态 / 错误态 / 正常态的视觉差异
3. **页面流转缩略图**（可选）：核心用户旅程的多屏缩略流程图

约定：
- 图片命名 `ui-understanding-[界面ID或简称].png`，存放于 `design/ai-understanding/` 目录
- 在 `ux-design.md`「AI 视觉理解验证图」章节引用，并标注对应界面 ID 与设计稿来源
- 每张图下方附「AI 理解要点」简要说明（顶部/内容区/底部组件与对应设计稿文件），便于 reviewer 快速比对
...
7. 提示下一步：
   - 若已生成 AI 视觉理解验证图（`design/ai-understanding/`）：**优先 review 验证图**，确认 AI 对 UI 结构的理解是否正确
...
- **ux 写入侧边界守护（必须）**：写入 `ux-design.md` 前须检查每条内容是否越界进入 spec 事实源——**行为契约**（「系统必须……」句式）、**量化指标**（超时阈值/百分比/并发数）、**范围定义**（In/Out Scope）、**状态进入/退出的业务规则**均归 `spec.md`；命中时**禁止直接写入**，按 block_ask 向用户列出条目并三选一：（a）移入 spec.md 补充清单（推荐）；（b）确认保留并说明理由；（c）拆分——呈现部分留 ux-design，行为部分移 spec。详见 `.cursor/rules/aisdd-document-boundaries.mdc` **Part B**
