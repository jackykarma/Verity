...
| **key** | §3 | `key-func-design.md` + `epic-design.md` §3 摘要引用 | 关键疑难点/亮点设计、方案流程图 |
| **diagram** | §4 | `key-diagram.md` + `epic-design.md` §4 摘要引用 | 全景类图、Feature 子类图、关键时序图 |
...
/aisdd.epicdesign 参数说明：
  ...
  key        产出关键功能疑难设计（§3）→ key-func-design.md
  diagram    产出全景类图与关键时序（§4）→ key-diagram.md
  story      产出 Story 拆解（§5）
...
| `key-func-design.md` | §3 | 关键功能疑难点/亮点设计、方案流程图、Feature 流程图集 |
...
4. **根据参数产出**：
   ...
   - **diagram**：产出 `key-diagram.md`，并更新 `epic-design.md` 的 §4 为摘要 + 引用该文件。
