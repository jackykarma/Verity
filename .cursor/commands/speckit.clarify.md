...
**定位与使用场景**：
- **clarify**：用于 **首次交互式需求澄清**，在 `/speckit.plan` **之前** 执行
...
- **不讨论技术方案**：不得提出技术选型、架构设计、实现方式、技术栈、存储/框架/接口设计等方案类问题；上述内容在 `/speckit.plan` 与 `/speckit.epicdesign` 阶段讨论
...
1. 从代码库根目录**执行一次** `.specify/scripts/powershell/check-prerequisites.ps1 -Json -PathsOnly`（组合模式 `--json --paths-only` / 简写 `-Json -PathsOnly`）。解析JSON返回结果中的核心字段：
    ...
    - 若JSON解析失败，终止操作并告知用户重新运行 `/speckit.feature`（创建 Feature 文档目录与 spec.md）或检查 `SPECIFY_FEATURE`/当前 EPIC 上下文是否正确。
...
- 若规格说明文件缺失，告知用户先运行 `/speckit.feature`（不在此处新建规格说明）。
