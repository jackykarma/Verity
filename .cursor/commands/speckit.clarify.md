...
**定位与使用场景**：
- **clarify**：用于 **首次交互式澄清**，在 `/speckit.plan` **之前** 执行
- **适用场景**：spec 初次生成后，存在模糊点、缺失关键信息、未明确决策点
- **交互方式**：AI 主动提出最多 5 个针对性问题，用户回答后整合回 spec
- **变更追溯**：在「澄清内容」章节记录问答，但 **不维护变更记录表**（因为是首次补全，非变更）

**与 feature-update 的区别**：
- 若已完成 clarify，后续需求有变化 → 使用 `/speckit.feature-update`（主动指定范围、维护变更记录、可级联 plan）
- clarify 是"首次填空"，feature-update 是"后续修改"
