---
description: 执行实施计划，处理并执行 tasks.md 中定义的所有任务（开发者在 Story 分支执行；严格遵循 EPIC 分支冻结的 spec/plan/tasks）
---
...
2. **检查检查清单状态**（若 FEATURE_DIR/checklists/ 目录存在）：
    ...
    - **若存在未完成的检查清单**：
        - 展示包含未完成项数的表格
        - **默认停止执行**（避免在质量门禁未满足时继续推进）
        - 仅当用户在本次命令输入（`$ARGUMENTS`）中**显式**包含“继续/强制/force”等明确意图时，才允许继续进入步骤 3（并在报告中注明“带风险继续执行”）
