...
| 阶段 | 参数 | 输出内容 | 适用场景 |
|------|------|----------|----------|
| **Lite**（默认） | `--phase=lite` 或不指定 | A0、A1、A2、A3.1、A3.2、Plan-B | 小改动/低风险功能 |
| **Standard** | `--phase=standard` | A3.3（组件详细设计）、Story Breakdown、A4-A11（风险/边界/评估） | 中等复杂度功能 |
...
| **A3.2** | 第二层：Feature 全景（流程图集 + 全景类图 + 关键时序图集 + 疑难点与亮点设计详解若适用） |
| **Plan-B** | 技术规约（B0-B7：技术背景、架构、数据模型、接口、合规、项目结构） |

#### Standard 阶段（组件详细设计 + Story Breakdown + 风险/评估）

| 章节 | 内容 |
|------|------|
| **A3.3** | 第三层：组件内部详细设计（复杂组件的详细类图 + 完整详细时序图 + 异常清单） |
...
- **Lite（默认）**：A0-A2、A3.1-A3.2、Plan-B
- **Standard**：Story Breakdown、A3.3（组件详细设计）、A4-A11（风险/边界/评估）
...
4. **根据阶段执行对应内容**：
   ...
   - **A3.1 第一层设计**：内部框架图 + 组件清单与职责 + 组件协作时序图（若存在 EPIC_ARCH，须继承 epic-arch 的 1 层架构与规范约束）
   - **A3.2 第二层设计**：Feature 流程图集 + 全景类图 + 关键时序图集 + 疑难点与亮点设计详解（若适用）
   ...
   - **A3.3 第三层设计**（组件详细设计）：对需要细化的组件产出详细类图 + 完整详细时序图 + 异常清单
