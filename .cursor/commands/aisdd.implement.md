...
**`-h` 帮助输出**（当且仅当参数为 `-h` 时输出，不执行）：

```text
/aisdd.implement 参数说明：

  (无参数) / all   执行 tasks.md 中全部未完成 Task
  ST-xxx           仅执行指定 Story 的未完成 Task（含依赖闭包；可多个）
  Txxx             仅执行指定 Task（含依赖闭包；可多个，如 T101 T102）
  -h               显示本帮助信息

依赖闭包：目标 Task 的「依赖」字段中引用的未完成 Task，递归展开直至无未完成的 T???。
已标记 [x] 的 Task 跳过，不重做（除非用户明确要求 --force，本工作流默认不支持 force）。
```
