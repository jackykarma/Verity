...
**提问方法论**（吸收 `.agents/skills/grilling/SKILL.md`，受上述边界与 5 问上限约束）：

- **决策树逐分支**：将 spec 中的待澄清点组织为需求决策树（根 → 分支 → 叶子），按**依赖顺序**推进——先解上游决策（用户角色、范围 In/Out、主流程），再解下游（异常、边界、NFR 口径、跨 Feature 依赖）；同一分支内 Partial/Missing 优先于已 Clear 的相邻分支
- **一次一问**：每次**仅提一个问题**，等待用户反馈后再继续；禁止一次抛出多个问题
- **每题带推荐答案**：无论开放题或选择题，均须给出**推荐答案/推荐选项及理由**（基于 spec 上下文、epic.md、行业惯例与合理默认值）；用户可直接采纳、修正或拒绝
- **能查则不问**：若答案可从存量代码、`features/FEAT-xxx/research/`、epic.md 或 spec 已有章节推断，**先自行查证**并在 spec 中记录为「假设」或补全内容，不占用提问配额；仅当存在多种合理且影响不同的解读、或无可靠默认值时才提问
- **单题追问不计入新问题**：对当前问题的澄清性追问（「你指的是 A 还是 B？」）不占用 5 问配额

**执行步骤**：

1. 运行 `check-prerequisites.ps1 -Json -PathsOnly`，解析 `FEATURE_DIR`、`FEATURE_SPEC`；spec 缺失则终止
2. 读取 `FEATURE_SPEC`、`epic.md`；Brownfield 时优先查阅本 Feature 的 `research/`（若存在）及可推断的存量代码，减少无谓提问
3. 按分类体系扫描模糊点（功能范围、领域模型、交互流程、NFR、集成依赖、边缘场景、术语、验收可测试性等），标记 Clear / Partial / Missing；构建需求决策树并识别分支间依赖
4. 自决策树根向叶生成优先级队列（**最多 5 个新问题**），取队首一题按「提问方法论」向用户提问
5. **顺序提问循环**：用户回答后整合至 spec，再取下一题；用户回复「完成」「停止」或达 5 问上限时结束
6. **每次整合前纯净度自检**（8 类污染清单，见 §spec / 技术细节边界守护）；命中则 block_ask，不得直接写入
7. 在 spec 中维护 `## 澄清内容` → `### 会话 YYYY-MM-DD` 记录问答；同步更新 FR/NFR/AC/场景等对应章节
8. **完成报告**：已解答问题数、涉及章节、覆盖度汇总（已解决/已推迟/清晰/未解决）、未展开分支（若有）、建议下一步
