...
**从story_detail_design.md中加载**（若存在，路径为 FEATURE_DIR/story_detail_design.md）：
- 各 Story 的「本 Story 覆盖的 L1 类」或类图中的类名
- 文档末尾的「L1 类覆盖矩阵」（若有）
...
#### G. L1 类覆盖检测（Plan Level = Deep 且 story_detail_design.md 存在时执行）
- 从 plan.md 的 A3.2.2 全景类图中提取 L1 类名列表
- 从 story_detail_design.md 的「L1 类覆盖矩阵」或各 Story 的类图/「本 Story 覆盖的 L1 类」中提取已覆盖的类
- 若存在 L1 类未被任何 Story 覆盖：报告 CRITICAL
- 若「L1 类覆盖矩阵」缺失或未填写完整：报告 HIGH

### 5. 严重程度赋值

采用以下启发式规则对发现结果排序：

- **CRITICAL（严重）**：违反章程MUST条款、核心规格工件缺失、阻碍基线功能实现且无覆盖的需求、**L1 类未被 Story L2 覆盖**（Plan Deep 时）
...
**L1 类覆盖情况**（Plan Level = Deep 且 story_detail_design.md 存在时填写）：

| L1 类（来自 A3.2.2） | 是否被 Story 覆盖 | 覆盖的 Story | 备注 |
|----------------------|-------------------|--------------|------|
| [类名] | 是/否 | ST-xxx |  |

若存在未覆盖的 L1 类，报告为 CRITICAL。
