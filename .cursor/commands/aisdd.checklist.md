---
description: 根据用户需求为当前功能生成需求质量检查清单，仅用于 spec-ready 前的需求评审，不作为 implement 阶段阻塞条件。
---
...
5. **生成检查清单** - 创建「需求的单元测试」：
    ...
    - 生成唯一的检查清单文件名：
        ...
        - 若文件已存在，追加内容至现有文件
    - 检查项编号从CHK001开始连续编排
    - 每次运行 `/aisdd.checklist` 均创建**新文件**（永不覆盖现有检查清单）
...
**重要说明**：每次调用 `/aisdd.checklist` 命令都会生成一个检查清单文件（除非文件已存在），文件名需简短且具描述性。这样做的目的：
