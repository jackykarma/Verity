...
**定位与使用场景**：
- **clarify**：用于 **首次交互式需求澄清**，在 `/aisdd.plan` **之前** 执行
...
- **不讨论技术方案**：不得提出技术选型、架构设计、实现方式、技术栈、存储/框架/接口设计等方案类问题；上述内容在 `/aisdd.plan` 与 `/aisdd.epicdesign` 阶段讨论
...
注意：该澄清流程需在调用 `/aisdd.plan` **之前** 完成。若用户明确表示跳过澄清（如探索性原型开发），可继续执行，但必须提示后续返工风险会增加。
...
1. 从代码库根目录**执行一次** `.specify/scripts/powershell/check-prerequisites.ps1 -Json -PathsOnly`（组合模式 `--json --paths-only` / 简写 `-Json -PathsOnly`）。解析JSON返回结果中的核心字段：
    ...
    - 若JSON解析失败，终止操作并告知用户重新运行 `/aisdd.feature`（创建 Feature 文档目录与 spec.md）或检查 `SPECIFY_FEATURE`/当前 EPIC 上下文是否正确。
...
8. 完成报告（提问循环结束或提前终止后）：
    ...
    - 若存在未解决或已推迟的分类，建议是否继续执行 `/aisdd.plan`，或在方案制定后再次运行 `/aisdd.clarify`。
    - 建议的下一步指令。

行为规则：

- 若未发现有意义的模糊点（或所有潜在问题均为低影响度），回复：“未检测到需正式澄清的核心模糊点。” 并建议继续推进。
- 若规格说明文件缺失，告知用户先运行 `/aisdd.feature`（不在此处新建规格说明）。
