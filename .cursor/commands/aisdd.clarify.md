...
5. 每接收一个有效答案后，立即整合至规格说明（增量更新方式）：
    - **写入前纯净度自检（强制，每次整合前执行）**：按 `aisdd.featurespec.md` §「spec / 技术细节边界守护」8 类清单扫描即将整合的答案文本及改写后的 FR/NFR/AC/场景条目；命中污染时**不得整合写入**，按 block_ask 向用户展示原文、命中类别与建议归属，请用户四选一：（a）改写为业务语言后留在 spec.md（推荐）；（b）移出 spec，记入 plan/epic-design 待办；（c）确认保留并说明理由；（d）拆分业务部分留 spec、技术部分移下游。详见 `docs/aisdd/spec-vs-plan-design-boundary.md`。
