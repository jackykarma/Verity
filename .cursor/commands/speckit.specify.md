---
description: 根据自然语言的功能描述创建或更新 Feature 规格说明（Epic/Feature/FR-NFR/依赖/边界/版本），为 Plan 阶段提供权威输入。
handoffs: 
  - label: 制定技术方案
    agent: speckit.plan
    prompt: 为该规格说明制定方案。我正在基于……进行开发
  - label: 澄清规格说明要求
    agent: speckit.clarify
    prompt: 澄清规格说明的相关要求
    send: true
---

## 用户输入

```text
$ARGUMENTS
```

在继续操作前，你**必须**参考用户输入（若不为空）。

## 大纲

用户在触发消息中 `/speckit.specify` 后输入的文本**即为** Feature 描述。即使下方出现字面量 `$ARGUMENTS`，也需假定该对话中始终可获取此描述。除非用户提交的指令为空，否则请勿要求用户重复描述。

根据该功能描述，执行以下操作：

1. **为分支生成简洁的短名称**（2-4 个单词）：
    - 分析功能描述并提取最具意义的关键词
    - 创建一个 2-4 个单词的短名称，精准概括功能核心
    - 尽可能采用“动作-名词”格式（例如："add-user-auth"、"fix-payment-bug"）
    - 保留专业术语和缩写（OAuth2、API、JWT 等）
    - 兼顾简洁性与描述性，确保一眼就能理解功能内容
    - 示例：
        - "我想添加用户认证功能" → "user-auth"
        - "为 API 实现 OAuth2 集成" → "oauth2-api-integration"
        - "创建数据分析仪表盘" → "analytics-dashboard"
        - "修复支付处理超时漏洞" → "fix-payment-timeout"

2. **创建新分支前检查现有分支**：

   a. 首先，拉取所有远程分支以确保获取最新信息：

      ```bash
      git fetch --all --prune
      ```

   b. 查找该短名称在所有来源中的最高功能编号：
    - 远程分支：`git ls-remote --heads origin | grep -E 'refs/heads/[0-9]+-<short-name>$'`
    - 本地分支：`git branch | grep -E '^[* ]*[0-9]+-<short-name>$'`
    - 规格说明目录：检查匹配 `specs/[0-9]+-<short-name>` 格式的目录

   c. 确定下一个可用编号：
    - 提取所有三个来源中的编号
    - 找到最大编号 N
    - 新分支编号使用 N+1

   d. 运行脚本 `.specify/scripts/powershell/create-new-feature.ps1 -Json "$ARGUMENTS"`，并传入计算出的编号和短名称：
    - 同时传入 `--number N+1`、`--short-name "your-short-name"` 以及功能描述
    - Bash 示例：`.specify/scripts/powershell/create-new-feature.ps1 -Json "$ARGUMENTS" --json --number 5 --short-name "user-auth" "Add user authentication"`
    - PowerShell 示例：`.specify/scripts/powershell/create-new-feature.ps1 -Json "$ARGUMENTS" -Json -Number 5 -ShortName "user-auth" "Add user authentication"`

   **重要说明**：
    - 必须检查所有三个来源（远程分支、本地分支、规格说明目录）以找到最大编号
    - 仅匹配与短名称模式完全一致的分支/目录
    - 若未找到包含该短名称的现有分支/目录，则从编号 1 开始
    - 每个功能仅允许运行一次该脚本
    - 终端会输出 JSON 内容——务必参考该内容获取所需的实际信息
    - JSON 输出将包含 BRANCH_NAME（分支名称）和 SPEC_FILE（规格文件）路径
    - 对于参数中包含单引号的情况（如 "I'm Groot"），需使用转义语法：例如 'I'\''m Groot'（或尽可能使用双引号："I'm Groot"）

3. 加载 `.specify/templates/spec-template.md` 以了解所需的章节内容。

4. 遵循以下执行流程：

    1. 解析输入中的用户描述
       若为空：输出错误信息 "未提供功能描述"
    2. 从描述中提取核心概念
       识别：目标用户/场景、价值、范围边界、依赖、数据实体、质量属性（NFR）
    3. 针对不明确的内容：
        - 基于上下文和行业标准做出合理推测
        - 仅在以下情况标注 [需澄清：具体问题]：
            - 该选择对功能范围或用户体验有重大影响
            - 存在多种合理且影响不同的解读方式
            - 无合理的默认值可参考
        - **限制条件**：最多标注 3 个 [需澄清] 标记
        - 按影响优先级排序需澄清的内容：范围 > 安全/隐私 > 用户体验 > 技术细节
    4. 填写 Feature 元信息（必须）
       - Epic：EPIC-[编号] - [名称]（若未知可先写 TODO(Epic) 并在 `/speckit.clarify` 解决）
       - Feature 类型：Product Feature 或 Platform/Capability Feature
       - Feature ID：优先沿用分支号（例如分支 `001-xxx` → `FEAT-001`）
       - Feature Version：初始必须为 `v0.1.0`
       - 状态：草稿/待澄清/就绪（可进入 Plan）
    5. 填写背景与价值、范围与范围外（必须）
       - 必须写清楚 In Scope / Out of Scope
    6. 填写依赖关系（必须）
       - 上游/下游影响/外部依赖故障模式（超时/限流/不可用/不一致）
    7. 填写验收与场景（必须）
       - 核心用户旅程（可选但建议）
       - 边界与异常场景（必填：数据/状态/生命周期/并发/用户行为）
    8. 生成 FR / NFR（必须）
       - FR：每条必须可测试，使用 FR-001… 编号
       - NFR：必须覆盖至少 性能/功耗/内存/安全隐私/可观测性/可靠性 六类；可暂用 [需澄清] 标记，但必须在 Plan 阶段量化并给出验收方法
    9. 定义验收标准（Feature Level）（必须）
       - 使用 AC-001… 编号，并引用对应 FR/NFR ID
    10. 识别核心实体（如涉及数据则必填）
    11. 填写假设与约束（必须）
    12. 返回结果：成功（Feature 规格说明已就绪，可进入澄清或 Plan 阶段）

5. 按照模板结构将规格说明写入 SPEC_FILE，替换占位符为从功能描述（参数）中提取的具体内容，同时保留章节顺序和标题。

6. **规格说明质量验证**：编写初始 `spec.md` 后，对照质量标准进行验证：

   a. **创建规格质量检查清单**：在 `FEATURE_DIR/checklists/requirements.md` 路径生成检查清单文件，包含以下验证项（与新模板对齐）：

      ```markdown
      # 规格说明质量检查清单：[功能名称]
      
      **目的**：在进入规划阶段前验证规格说明的完整性和质量
      **创建时间**：[日期]
      **功能**：[规格说明文件链接]
      
      ## 内容质量
      
      - [ ] 无实现细节（语言、框架、API）
      - [ ] 聚焦用户价值和业务需求
      - [ ] 面向非技术相关方编写
      - [ ] 所有必填章节已完成
      
      ## 结构与元信息

      - [ ] 已填写 Epic、Feature 类型、Feature ID、Feature Version、状态
      - [ ] Feature Version 初始为 v0.1.0（后续变更走增量变更流程）

      ## 需求完整性（Feature）
      
      - [ ] 无剩余 [需澄清] 标记
      - [ ] 需求可测试且无歧义
      - [ ] 成功标准可衡量
      - [ ] 成功标准与技术无关（无实现细节）
      - [ ] 背景与价值、In Scope、Out of Scope 已完整填写
      - [ ] 依赖关系已填写（上游/下游/外部依赖故障模式）
      - [ ] 边界与异常场景已定义（数据/状态/生命周期/并发/用户行为）
      - [ ] FR 采用编号（FR-xxx）且每条可测试
      - [ ] NFR 覆盖性能/功耗/内存/安全隐私/可观测性/可靠性（允许少量 [需澄清]，但不得缺失整类）
      - [ ] 验收标准（AC-xxx）已定义并引用 FR/NFR
      - [ ] 已识别核心实体（如适用）
      - [ ] 已记录假设与约束
      
      ## 就绪度（可进入 Plan）
      
      - [ ] 核心模糊点已收敛（剩余 [需澄清] ≤ 3 且不阻塞 Plan）
      - [ ] 无实现细节渗入规格说明（不写框架/代码结构/API 实现方式）
      - [ ] 术语一致（Epic/Feature/FR/NFR/AC 的命名一致）
      
      ## 备注
      
      - 未完成的项需更新规格说明后，方可执行 `/speckit.clarify` 或 `/speckit.plan` 命令
      ```

   b. **执行验证检查**：对照检查清单的每一项审核规格说明：
    - 确定每一项的通过/失败状态
    - 记录发现的具体问题（引用规格说明的相关章节）

   c. **处理验证结果**：

    - **若所有项通过**：标记检查清单为完成，并进入步骤 6
    - **若有项失败（不含 [需澄清] 标记）**：
        1. 列出失败项及具体问题
        2. 更新规格说明以解决每个问题
        3. 重新执行验证，最多迭代 3 次
        4. 若 3 次迭代后仍有失败项，在检查清单备注中记录剩余问题并向用户发出警告
    - **若仍有 [需澄清] 标记**：
        1. 提取规格说明中所有 [需澄清：……] 标记
        2. **数量限制检查**：若标记超过 3 个，仅保留影响最重大的 3 个（按范围/安全/用户体验优先级），其余部分基于合理推测补充
        3. 对每个需澄清的内容（最多 3 个），按以下格式向用户呈现选项：

           ```markdown
           ## 问题 [编号]：[主题]
           
           **背景**：[引用规格说明的相关章节]
           
           **需要确认的内容**：[来自“需澄清”标记的具体问题]
           
           **建议答案**：
           
           | 选项 | 答案 | 影响 |
           |--------|--------|--------------|
           | A      | [第一个建议答案] | [该选项对功能的影响] |
           | B      | [第二个建议答案] | [该选项对功能的影响] |
           | C      | [第三个建议答案] | [该选项对功能的影响] |
           | 自定义 | 提供您自己的答案 | [说明如何提交自定义输入] |
           
           **您的选择**：_[等待用户回复]_
           ```

        4. **关键要求 - 表格格式**：确保 Markdown 表格格式正确：
            - 竖线对齐，间距一致
            - 每个单元格内容两侧保留空格：`| 内容 |` 而非 `|内容|`
            - 表头分隔线至少包含 3 个连字符：`|--------|`
            - 验证表格在 Markdown 预览中可正常渲染
        5. 问题按顺序编号（Q1、Q2、Q3 - 最多 3 个）
        6. 一次性呈现所有问题，再等待用户回复
        7. 等待用户回复所有问题的选择（例如："Q1: A, Q2: 自定义 - [具体内容], Q3: B"）
        8. 根据用户选择或提供的自定义答案，更新规格说明，替换对应的 [需澄清] 标记
        9. 所有澄清内容解决后，重新执行验证

   d. **更新检查清单**：每次验证迭代后，更新检查清单文件的通过/失败状态

7. 报告完成情况，包括分支名称、规格文件路径、检查清单结果，以及是否就绪进入下一阶段（`/speckit.clarify` 或 `/speckit.plan`）。

**注意**：脚本会先创建并切换到新分支，初始化规格文件后，再写入内容。

## 通用指南

### 快速指南

- 聚焦用户**需要什么**以及**为什么需要**。
- 避免描述**如何**实现（不提及技术栈、API、代码结构）。
- 面向业务相关方编写，而非开发人员。
- 请勿在规格说明中嵌入任何检查清单，检查清单需通过单独命令生成。

### 章节要求

- **必填章节**：每个功能都必须完成
- **可选章节**：仅在与功能相关时包含
- 若某章节不适用，直接删除（勿保留为 "不适用"）

### AI 生成规则

基于用户提示创建规格说明时：

1. **合理推测**：结合上下文、行业标准和通用模式填补信息空白
2. **记录假设**：在“假设”章节记录所使用的合理默认值
3. **限制澄清次数**：最多使用 3 个 [需澄清] 标记——仅用于以下关键决策：
    - 对功能范围或用户体验有重大影响
    - 存在多种合理且影响不同的解读方式
    - 无任何合理默认值可参考
4. **澄清优先级**：范围 > 安全/隐私 > 用户体验 > 技术细节
5. **以测试视角思考**：任何模糊的需求都应判定为“不可测试、有歧义”（不符合检查清单要求）
6. **常见需澄清场景**（仅在无合理默认值时）：
    - 功能范围和边界（包含/排除特定用例）
    - 用户类型和权限（若存在多种冲突解读）
    - 安全/合规要求（涉及法律/财务重大影响时）

**合理默认值示例**（无需询问）：

- 数据留存：所属领域的行业通用做法
- 性能指标：除非特别说明，否则采用标准 Web/移动应用的预期值
- 错误处理：用户友好的提示信息及适当的降级方案
- 认证方式：Web 应用默认采用基于会话或 OAuth2 认证
- 集成模式：除非特别说明，否则默认采用 RESTful API

### 成功标准编写指南

成功标准必须满足：

1. **可衡量**：包含具体指标（时间、百分比、数量、速率）
2. **与技术无关**：不提及框架、语言、数据库或工具
3. **以用户为中心**：从用户/业务视角描述结果，而非系统内部逻辑
4. **可验证**：无需了解实现细节即可测试/验证

**正面示例**：

- "用户可在 3 分钟内完成结账流程"
- "系统支持 10,000 个并发用户"
- "95% 的搜索请求可在 1 秒内返回结果"
- "任务完成率提升 40%"

**反面示例**（聚焦实现细节）：

- "API 响应时间低于 200 毫秒"（过于技术化，应改为“用户可即时看到结果”）
- "数据库可处理 1000 TPS"（实现细节，应改为面向用户的指标）
- "React 组件渲染高效"（框架相关）
- "Redis 缓存命中率高于 80%"（技术相关）