...
- 问题：首版可观测性：听力训练是否需要在首版做产品埋点？ → 答案：首版不做产品埋点，仅保留技术日志（崩溃、异常），产品数据后续迭代补。
- 问题：单份听力材料与题目的数量关系？ → 答案：一对多（1 份材料对应多道题）。
- 问题：首批听力材料与题目的来源与规模？ → 答案：未定，Plan 阶段再定。
- 问题：功耗（NFR-POWER-001）在首版是否需要量化上限？ → 答案：不需要，沿用 EPIC 级；首版不做功耗量化。
- 问题：进入听力训练时，材料列表「加载中」或「为空」应如何展示？ → 答案：未定，uidesign 再定。
...
- 无可用材料或题目 → 提示“暂无可练内容”或引导至其他模块；不崩溃。材料列表加载中、为空时的具体 UI 形态（骨架屏/转圈/空状态页等）在 uidesign 阶段确定；本 spec 仅约束有明确提示、不崩溃。
...
- **NFR-POWER-001**：首版不做功耗量化，沿用 EPIC 级约束；常规训练会话下无明显异常耗电即可。若后续需量化则 plan 或迭代中补充。
...
- **NFR-OBS-001**：首版不做产品埋点，仅保留技术日志（崩溃、异常等）；播放失败、题目提交等关键异常可记入技术日志以便排查。产品埋点后续迭代补。
...
- **听力材料**：含义：一份可播放的音频及其元数据（标题、时长、难度等）；核心属性：音频资源引用、题目或练习关联；与题目为一对多（1 份材料对应多道题）。
- **题目/练习**：含义：与材料配套的作答单元；核心属性：题型、题干、选项或答案、解析；多道题目归属于同一份材料，提交后生成作答记录。
- **作答记录 / 结果**：含义：用户某次练习的提交与反馈；核心属性：正误、得分、错题或解析；用于结果展示与后续统计（若 EPIC 有统一进度）。

## 假设与约束 *（必填）*

- **假设**：设置 Feature 已提供发音偏好读写接口；设备具备扬声器或耳机输出。首批材料来源（预置/下载包）与规模（份数、题目数）在 Plan 阶段确定；当前设计假设为预置或 bundled、格式为系统可播。若假设不成立：无偏好时使用缺省（如美式）；无音频输出时仅能做题目、无播放；格式不支持时按故障模式降级。
