...
当前覆盖: PERF(3), MEM(2), SEC(3), COMPAT(3), OBS(3), REL(3) ✓
...
- [ ] **数据加载速度**：首屏数据加载 < 2 秒（EPIC 关键关注点）✓
- [ ] **图片显示速度**：缩略图显示 < 300ms（EPIC 关键关注点）✓
- [ ] **内存预算**：符合 EPIC 内存预算（运行时 < 300MB，缓存 < 150MB）✓
...
当前实体: MediaItem, Album, PermissionState ✓
...
- [ ] **接口清晰**：提供的接口/契约清晰易用（如 Repository 接口）
- [ ] **可复用性**：设计考虑了复用性，避免与特定业务 Feature 耦合
- [ ] **独立验收**：可以独立于业务 Feature 验收能力本身

### 权限管理特定检查

- [ ] **权限最小化**：仅申请必要权限，符合 Android 最佳实践
- [ ] **用户引导友好**：权限被拒绝时提供清晰的说明和引导
- [ ] **版本适配完整**：覆盖了 Android 10-14 的权限模型差异
- [ ] **隐私合规**：符合隐私政策，不上传用户数据
