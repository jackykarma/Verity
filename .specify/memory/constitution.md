...
- **Plan-Lite（小改动/低风险）**：至少覆盖 A2（全景边界）、A3.1（组件清单）、A3.2（流程图集 + 全景类图 + 时序图集 + 疑难点与亮点设计详解若适用）；NFR 预算可用最小集说明。
- **Plan-Standard（默认）**：在 Lite 基础上补齐 Story Breakdown、A3.3（组件内部详细设计）、A4-A11（技术风险、边界异常、算法/功耗/性能/内存/安全/兼容性评估）
- **Plan-Deep（高风险/高不确定性）**：在 Standard 基础上，补齐**所有** Story 的 L2 细化设计（按 Story 序号覆盖，不得遗漏）
