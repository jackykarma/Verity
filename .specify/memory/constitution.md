...
- **Plan-Lite（小改动/低风险）**：至少覆盖 A2（全景边界）、A3.1（组件清单）、A3.2（全景类图 + 时序图集 + 流程图集）；NFR 预算可用最小集说明。
- **Plan-Standard（默认）**：在 Lite 基础上补齐 Story Breakdown、风险/预算落点；A3.3（组件内部详细设计）**可选**，仅当组件内部复杂度足够高时补充
- **Plan-Deep（高风险/高不确定性）**：当涉及新契约/持久化迁移/复杂动效与性能预算/并发竞态/灰度回滚等，需补齐 A3.3（组件内部详细设计）与关键 Story 的 L2 细化
