...
> **定位**：EPIC/主 Feature 级的**结构纵览与主流程说明**，服务于方案评审与 Task 对设计的引用。本节有类图、时序图，但粒度是「全景、关键流程」，**不等于**按 Story 的落码级详细设计；落码级详细设计在 §十三 所索引的 L2（`story_detail_design.md`）中。
>
...
>
>
> | 维度     | §七 本节                                        | §十三 L2（story_detail_design.md） |
> | ------ | -------------------------------------------- | ------------------------------ |
...
> **范围**：每个 Feature 仅挑选 **1-2 个最关键/最复杂的流程**绘制时序图，简单流程、常规 CRUD 等不在此列——更多时序图在 §十三 L2（`story_detail_design.md`）中按 Story 粒度补充。
>
