...
> **定位**：EPIC/主 Feature 级的**结构纵览与主流程说明**，服务于方案评审与 Task 对设计的引用。本节有类图、时序图，但粒度是「全景、关键流程」，**不等于**按 Story 的落码级详细设计；落码级详细设计在 §十 所索引的 L2（`story_detail_design.md`）中。
>
> **目的**：从 EPIC 或主 Feature 纵览关键类/接口关系与主流程时序，便于评审与 Task 引用。须基于本工程实际架构与真实代码。
>
> **与 §十 L2（二层详细设计）的区别**：
>
> | 维度 | §七 本节 | §十 L2（story_detail_design.md） |
> |------|----------|----------------------------------|
> | 层级 | EPIC / 主 Feature 级 | Story 级（按 ST-xxx 分节） |
> | 类图/时序图 | 骨架类图（无方法签名）+ 按 Feature 分组的子类图（含方法签名）+ 主流程时序图 | 本 Story 的完整详细类图/时序图，不得引用上层 |
> | 目的 | 评审用整体视图；Task 引用设计所在章节 | tasks/implement 的详细设计事实源，落码级指导 |
> | 何时必做 | 设计说明书必有本节 | 仅当 Story 技术复杂度高或需落码级指导时补充 L2 |
>
> **图表规范**：
>> - **全景骨架类图（§7.2）**：仅展示跨 Feature 接口/抽象类与各 Feature 核心入口类之间的依赖关系，**不含方法签名**，让评审者快速理解整体结构
>> - **按 Feature 分组的子类图（§7.2.x）**：每个 Feature 一张类图，须覆盖该 Feature 的关键类/接口及**完整方法签名**（方法名 + 参数 + 返回值），依赖方向正确（上层依赖下层）
>> - **时序图（§7.3）**：每个 Feature 仅挑选 1-2 个最关键流程，participant 使用真实类名，覆盖正常 + 关键异常（alt/else）；更多时序图在 L2 中按 Story 补充
>>
>> 参见 `.cursor/rules/specify-diagram-requirements.mdc`。
...
> - 用 `namespace` 或注释标注所属 Feature，便于对照 §5.2 组件清单
> - 依赖方向正确（上层依赖下层）
...
> **范围**：每个 Feature 仅挑选 **1-2 个最关键/最复杂的流程**绘制时序图，简单流程、常规 CRUD 等不在此列——更多时序图在 §十 L2（`story_detail_design.md`）中按 Story 粒度补充。
>
> **端到端完整性（必须）**：每张时序图须根据**本工程实际架构与真实代码**绘制**完整调用链路**，不得省略关键方法调用。
> - **participant 与调用链**：按工程真实分层与类来画——若架构中不存在 VM/UseCase/算法/Repository 等某一层，则图中不出现该层；存在几层就画几层，从触发端到最终响应/持久化/网络等真实终点**无断链**。
> - **异常分支**：alt/else 中的异常处理路径也须画出**实际代码中的调用链**（哪些类、哪些方法被调用），不得只写“失败”而缺具体调用。
>
> **粒度**：participant 使用**真实类名**，消息使用**真实方法名（可带关键参数）**。
>
> **关联**：
> - **关联 §六**：若时序图对应 §六 中的某个疑难点/亮点，在索引表中标注"关联 §6.x"，便于评审时从策略到实现细节的衔接
> - **关联 §7.1**：在索引表中标注对应的 §7.1 流程图，确保逻辑流程与方法调用时序一一对应

#### 时序图索引

| Seq ID | 所属 Feature | 流程名称 | 对应 §7.1 流程 | 覆盖的异常（EX-xxx） | 关联 §六 |
|--------|-------------|----------|----------------|----------------------|----------|
| SEQ-001 | FEAT-001 | [流程名称] | 流程 1 | EX-001, EX-002 | §6.x / — |
| SEQ-002 | FEAT-002 | [流程名称] | 流程 2 | EX-003 | — |
| SEQ-003 | 跨 Feature | [流程名称] | — | — | §6.x / — |

#### SEQ-001：[流程名称]（FEAT-001）

> **绘制要求**：participant 为本 Feature 内**真实类名**，消息为**真实方法调用**；从触发端到终点完整无断链；异常分支画出实际代码中的调用链（如 Repo 调用的错误转换、VM 调用的降级逻辑等）。
