...
| `key-func-design.md` | §六 | 关键功能疑难点/亮点设计策略、各设计点**核心方案**（清晰详述设计与实现；涉及技术点与**相关技术链路**须全覆盖，链上**每一环如何达成**须可核对）、方案流程图、核心调用链时序图、Feature 流程图集（**每张时序图紧下方须有详细协作者与过程说明**，见 `specify-diagram-requirements.mdc` §四；**核心方案**须与图表互证，模板见 `key-func-design-template.md`） |
...
> **定位**：面向方案评审，将重难点模块的**设计策略与决策逻辑**讲清楚。每个设计点同时给出**流程图**（验逻辑分支完备性）和**核心调用链时序图**（验方案主干可行性），使 §六 阶段即可判断方案是否 ok——通过后再进入 §七 精确化。
>
...
→ **`key-func-design.md`**（含关键设计策略、各 KD **核心方案**、方案流程图、核心调用链时序图、Feature 流程图集）
...
> **定位**：在 §六 确认方案可行后，本节将设计**精确化到可编码级别**——全景类图（含完整方法签名）+ 完整时序图（穷举所有异常分支），供 Task 拆解与 implement 阶段直接引用。
>
> **与 §六 的分工**：§六 验方案主干（核心调用链通不通），§七 补全精度（方法签名对不对得上、所有异常都覆盖了吗）。
>
...
- 所有对外接口（§9.1）已在对应 Feature 的 `plan.md` §四 中有完整规约定义
...
- **FR 覆盖**：本 Feature 下所有 FR/NFR 是否已在 §12.6 覆盖矩阵中被至少一个 Story 覆盖？（无未覆盖的 FR）
...
| 九、接口设计                            | plan.md §四 能力承诺与约束规则对齐 | 接口设计事实源；Task 接口实现依据                             |
| 十、数据库设计                           | plan.md §三 数据存储约束（§3.1/§3.2）对齐   | 数据表结构参考；Task 数据层实现依据                           |
| 十一、埋点技术方案                         | —                 | 埋点事件参考                                         |
