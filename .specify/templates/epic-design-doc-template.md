...
| `key-func-design/` 目录 | §七 | 各关键设计**独立文件**，命名 **`KD_${三位序号}_${精炼slug}.md`**（例：`KD_001_session_framework.md`）；**每个 KD 文件内须包含**：核心方案、**关键类图**（全量公共方法签名）、**核心调用链时序图**（穷举关键异常分支），按需可加方案架构图。跨 KD / 跨 Feature 流程须在相关 KD 中互链说明，不再单独产出流程图集。撰稿见 **`key-func-design-kd-template.md`**；**每张时序图紧下方须有详细协作者与过程说明**（见 `specify-diagram-requirements.mdc` §四） |
...
> **Lite / Fast Track**：可省略本表；P0 覆盖在 §十二.6 FR/NFR 覆盖矩阵与 KD/L2 中引用 `SC-xxx` 即可。
>
> **Standard / Deep**：从各 Feature `spec.md`「完整场景矩阵」提取 **P0**，在设计过程中标注覆盖位置；完成后无「未覆盖」项。
...
> **Standard / Deep**：可在下表外补充 1～2 段说明。**Lite / Fast Track**：仅填表 + 链接，不复制 `epic.md` 正文。
...
> **Lite / Fast Track**：各 Feature 需求以索引为准，**禁止**在此复述 FR/NFR/场景全文。

### 2.1 需求背景

| 来源 | 相对路径 | 要点（可选） |
|---|---|---|
| EPIC | [`epic.md`](./epic.md) §背景与价值 | [一行或「见原文」] |

### 2.2 需求范围及限制（按 Feature 索引）

| Feature ID | spec 路径 | In Scope 摘要 | Out of Scope 摘要 | 关键 NFR 引用 |
|---|---|---|---|---|
| FEAT-001 | [`features/FEAT-001-xxx/spec.md`](./features/FEAT-001-xxx/spec.md) | [一行] | [一行] | [如 NFR-PERF-001；或「见 spec」] |
| FEAT-002 | [`features/FEAT-002-xxx/spec.md`](./features/FEAT-002-xxx/spec.md) | | | |

> **Standard / Deep**：可在上表后补充 EPIC 级适用范围说明（机型/地区/版本等），且不得与 spec 冲突。
...
> **Lite / Fast Track**：业务场景以各 `spec.md`「完整场景矩阵」的 `SC-xxx` 为准；本节**不重复**枚举 7 类场景，仅在 §6.2.1 补充**技术层**对策并与 KD 时序 `alt/else` 互校。
>
> **Standard / Deep**：可在此列出 spec 未覆盖的、设计层才可见的技术边界要点（每条须关联 `SC-xxx` 或标注「设计补充」）。

#### 6.2.1 场景 → 应对措施对照表（必须）

> 目的：把 spec 中的 `SC-xxx`（尤其 P0）落到可执行技术/设计对策，并与 §七 KD 时序图异常分支互校。
>
> **对策类型说明**：
>
...
> **定位**：面向方案评审，**提前验证关键疑难点的技术 / 算法可行性链路并锁定关键设计**。每个设计点须在其 **`key-func-design/KD_*_*.md` 文档内**直接给出**核心方案**（讲清技术实现链路）、**关键类图**（全量公共方法签名，验类型层面可行性）与**核心调用链时序图**（穷举关键异常分支，验协作主干），使 §七 阶段即可判断方案是否 ok。可按需补充**方案架构图**。跨 KD / 跨 Feature 流程须在相关 KD 中互链说明，不再单独产出流程图集。
>
> **核心方案（各 KD 必填）**：用清晰易懂的中文把**核心技术方案与实现思路**讲明白；须覆盖影响可行性的关键技术点与关键链路，并对链上**关键环节说明如何达成**（机制、职责、数据/状态传递与约束）；**与**该 KD 文件内**关键类图**、**核心时序图**（及若有的方案架构图）**互证**，不得仅用标题或条目堆砌代替叙述。细则见 `.specify/templates/key-func-design-kd-template.md`。
>
...
- **硬约束**：依赖无环；可并行 Story 改动路径不重叠；遵循 §五 架构，不新增未定义组件

### 12.2 拆解说明（本 EPIC）

> 用 1～3 句话说明本 EPIC 采用的拆解维度与并行策略（勿复制外部指南全文）。
