---
description: "Change Request（CR）变更请求模板：用于需求/交互/视觉/技术方案变更的统一入口，产出影响分析与下游更新清单"
---

# Change Request（CR）：[标题]

> **用途**：作为任何变更的**唯一入口**。先把“改了什么/为什么/影响谁/要更新哪些产物”说清楚，再进入增量更新与实现。
>
> **适用变更**：需求（Scope/FR/NFR/AC 的新增、修改、删除；含**中途新增需求**）、交互（流程/状态/反馈）、视觉&动效、技术方案（Plan 决策）。
>
> **走流程**：推荐运行 `/aisdd.cr` 命令自动执行（影响分析 → 生成 CR → 分步更新下游产物）；或手动填写本 CR 并评审通过后，按类型走 [workflow-overview.md §七](../workflow-overview.md#七变更管理)——需求类走 7.1，技术方案类走 7.2，按「下游更新清单」逐项更新产物。

**CR-ID**：CR-[YYYYMMDD]-[短编号]  
**类型**：需求 / 交互 / 视觉&动效 / 技术方案 / 混合  
**发起人**：[姓名/角色]  
**日期**：[YYYY-MM-DD]  
**关联**：[PR/Issue/会议纪要链接（可选）]

---

## 1) 变更内容（What changed）

- **新增**：
  - …
- **修改**：
  - …
- **删除**：
  - …

> 建议：用 ID/引用定位到事实源，例如 `FR-001`、`AC-002`、`NFR-PERF-001`、`ux-design.md:页面/流程ID`、`plan.md:A2/A3/ST-xxx`。

---

## 2) 变更原因与证据（Why）

- **原因**：[用户反馈/数据结论/约束变化/技术不可行/设计评审结论…]
- **证据**：[数据图/日志/复现步骤/对比截图/性能报告/竞品分析…]

---

## 3) 影响分析（Impact）

### 3.1 影响范围（对象）

- **EPIC/Feature**：EPIC-…；FEAT-…（可多选）
- **Spec（FR/NFR/AC/边界）**：FR-… / NFR-… / AC-… / 边界/异常…
- **UX（页面/流程/动效）**：UI-… / Flow-… / Motion-…
- **Plan（约束/能力边界）**：§一～§四…；设计说明书：Story 拆解/L2/接口/数据库…
- **Story/Task**：ST-… / T…

### 3.2 风险与回滚

- **风险**：[性能/功耗/内存/兼容性/稳定性/工期/依赖…]
- **是否需要回滚**：是/否
- **回滚策略**：[开关/灰度/降级/数据迁移回退/兼容策略…]

---

## 4) 下游更新清单（Update checklist）

> 规则：**只更新受影响范围**；每次更新必须记录 Version 与变更记录。
>
> **spec.md 禁触红线（不可豁免）**：`spec.md` 是**纯粹的产品规格**，下表中所有勾选项写入 spec.md 时，**绝对禁止**附带类名、接口、框架名、库名、表/字段、SQL、API 路径、代码片段、包路径、文件路径、线程原语、埋点字段等技术实现细节；技术决策属于 `plan.md` / `epic-design.md` / 其子文件。详见 `docs/aisdd/spec-vs-plan-design-boundary.md`。

- [ ] 更新 EPIC `ux-design.md`（如涉及交互/视觉/动效）
- [ ] 更新 Feature `spec.md` — **FR / AC / 范围（In/Out） / 用户旅程 / 边界与异常场景**（需求文字层面）
- [ ] 更新 Feature `spec.md` — **仅 NFR 量化指标**（如 `p95 ≤ Xms`、`内存 ≤ XMB`，**不写实现方案**；技术方案类 CR 涉及此项时必须显式勾选）
- [ ] 更新 Feature `plan.md`（如涉及方案决策/契约/预算/风险/Story；类名、接口、框架、库、模块、表/字段、API 路径等技术决策**全部**归此或更下游）
- [ ] 更新 `epic-design.md` 与 KD / 接口 / 数据库 / 埋点 / l2 等子文件（如涉及架构、详细设计、接口契约、表结构、埋点字段）
- [ ] 重新生成/更新 Feature `tasks.md`（确保验证步骤与追溯一致）

---

## 5) 结论与决策（评审后填写）

- **结论**：通过 / 有条件通过 / 不通过
- **待办**：[阻塞项与负责人]
- **签字/确认**：[产品/设计/技术/测试负责人]

