...
>
> **关联**：按需 [`feature-flows.md`](./feature-flows.md)（**仅**作跨 KD/跨 Feature 补充；**不可替代**下文「方案流程图」）
>
...
>
> **路径覆盖**：主干成功路径 + **所有异常分支**（alt/else 完整覆盖）。本节时序图是 KD 阶段的最终产出，须穷举全分支；L2 Story 时序在此基础上切片细化，不重复。
...
- **精确化与补全**：各 Story 的 L2 详细设计（`l2_design/ST-xxx_*.md`）在本 KD 类图/时序基础上，按 Story 切片补充本 Story 的完整类图/时序（不得与本节内容重复）。
