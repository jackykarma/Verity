...
>
> **关联**：[`key-diagram.md`](../key-diagram.md) §八 | 按需 [`feature-flows.md`](./feature-flows.md)（**仅**作跨 KD/跨 Feature 补充；**不可替代**下文「方案流程图」）
>
...
>
> **与 `key-diagram.md`（§八）的分工**：
>
> | 维度 | 本节「关键类图」（§七 / 本 KD 文档） | §八 `key-diagram.md` |
> |---|---|---|
> | **范围** | 仅本 KD 方案**离不开**的类/接口（宜控制在「能讲清主干」的最小集合） | Feature/EPIC 级**补全**：子类图、**全量**公共 API、字段与方法签名、变更标识（<<新增>>/<<修改>>）、跨 Feature 骨架等 |
...
- **精确化与补全**：→ `key-diagram.md` §8.2 [相关类/接口名] + SEQ-[xxx]（在**本节关键类图 / 核心时序**基础上，补全**全量**签名、变更标识、全景/子类图及**全分支**时序）
