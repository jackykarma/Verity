...
>
> **写入范围**：全局技术栈锁定、跨 Feature 依赖方向、共享能力 Owner、NFR 总预算、统一错误/日志/安全原则、Feature plan 执行顺序。
>
> **禁止写入**：0/1 层架构图、组件清单、类图、时序图、流程图、接口方法签名、数据库表字段、Story 拆解、L2 设计。上述内容进入 `/aisdd.epicdesign` 及其子文件。
>
...
- **详细设计位置**：方法签名、字段定义、错误码枚举在 `interface-design.md` 或 L2 中设计。
