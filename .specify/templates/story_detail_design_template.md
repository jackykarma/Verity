...
- **禁止省略式引用**：所有 Story 级落码细节须在**本文件内**完整书写，不得使用「见 A3」「同上」「参见 tech-spec.md/KD」等替代正文；允许引用关联 KD 作为上游设计事实源，但必须在本文复述并细化本 Story 继承的关键决策。
- **禁止偏离 KD**：凡与关联 KD 重叠的技术方案，本文只能细化触发条件、局部方法调用、字段转换、UI/边界分支和 Story 专属异常；不得重定义 KD 已确定的架构分层、核心接口、状态机、持久化策略、并发策略或错误处理策略。

**图表完整性**：
- **类图**：须为**完整详细**的类图，覆盖本 Story 涉及的全部关键类、接口与方法签名；**不得引用** tech-spec.md 旧版 plan 组件设计（A3，已废止）的类图。
- **时序图**：须为**完整详细**的时序图，覆盖正常流程与所有关键异常分支；**不得引用** tech-spec.md 组件设计的时序图。
...
> **完整性**：须在本文档内绘制**完整详细**的类图，覆盖本 Story 涉及的全部关键类、接口与方法签名；**不得引用** tech-spec.md 旧版 plan 组件设计（A3，已废止）的类图。
