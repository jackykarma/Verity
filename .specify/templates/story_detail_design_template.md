...
>
> **必须继承 KD 关键设计**：若本 Story 涉及 `key-func-design/KD_*_*.md` 中已定义的关键能力、核心类、接口契约、状态流转、异常策略或跨模块协作，L2 只能在该 KD 方案基础上做 Story 级细化，不得另起一套技术方案。若发现 KD 无法支撑本 Story，须先更新对应 KD 或发起设计变更，再继续撰写 L2。

| 类型 | 说明 |
| --- | --- |
| **关联 KD** | [无则填「—」；若有，列出 KD 编号与相对路径，如 `../../../key-func-design/KD_001_session_framework.md`，并写清本 Story 继承的关键决策] |
...
- **禁止省略式引用**：所有 Story 级落码细节须在**本文件内**完整书写，不得使用「同上」「参见 tech-spec.md/KD」等替代正文；允许引用关联 KD 作为上游设计事实源，但必须在本文复述并细化本 Story 继承的关键决策。
- **禁止偏离 KD**：凡与关联 KD 重叠的技术方案，本文只能细化触发条件、局部方法调用、字段转换、UI/边界分支和 Story 专属异常；不得重定义 KD 已确定的架构分层、核心接口、状态机、持久化策略、并发策略或错误处理策略。

**图表完整性**：
- **类图**：须为**完整详细**的类图，覆盖本 Story 涉及的全部关键类、接口与方法签名；详细设计归属 `epic-design.md` / 关联 KD / 本 L2 文件，不得从 `tech-spec.md` 引用类图。
- **时序图**：须为**完整详细**的时序图，覆盖正常流程与所有关键异常分支；详细设计归属 `epic-design.md` / 关联 KD / 本 L2 文件，不得从 `tech-spec.md` 引用时序图。
- **KD 对齐**：若类图/时序图涉及关联 KD 中已出现的类、接口、participant、方法或分支，名称、依赖方向、调用职责与异常语义必须与 KD 保持一致；L2 可增加 Story 局部类、方法和分支，但不得与 KD 冲突。
...
- [关联 KD 继承说明：若本 Story 关联 KD，逐条说明继承了哪些 KD 决策（核心类/接口、状态流转、异常策略、并发或持久化约束等），以及本 Story 只在哪些局部做细化；若不关联任何 KD，写明 `N/A：本 Story 不涉及 §七关键设计中的关键能力`]
...
> **完整性**：须在本文档内绘制**完整详细**的类图，覆盖本 Story 涉及的全部关键类、接口与方法签名；详细设计归属 `epic-design.md` / 关联 KD / 本 L2 文件，不得从 `tech-spec.md` 引用类图。
> **KD 对齐**：涉及关联 KD 的类/接口时，必须沿用 KD 的真实类名、接口名、公共方法签名和依赖方向；只允许补充本 Story 专属的局部类、方法或字段。若需要改变 KD 中的核心类或契约，须先更新 KD，不得在 L2 中单独改写。
...
> **完整性**：须在本文档内绘制**完整详细**的时序图，覆盖正常流程与**所有**关键异常分支；**不得引用** tech-spec.md 组件设计的时序图。
> **覆盖要求**：从触发到响应的完整方法调用链，不得遗漏核心交互步骤；须使用 `alt/else` 明确区分正常与异常分支。
> **KD 对齐**：若主流程或异常分支已在关联 KD 中定义，L2 时序图必须沿用 KD 的 participant、调用职责、成功/失败语义，并在其基础上补充 Story 专属触发、局部处理、字段转换和 UI/边界响应；不得另画一套与 KD 不一致的调用链。
