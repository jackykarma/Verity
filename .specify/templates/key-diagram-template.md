...
> **与 §六 核心时序的区别**：§六 `key-func-design.md` 中的核心调用链时序图只画主干成功路径 + 关键异常，目的是验方案可行性。本节在此基础上**精确化 + 补全**——使用真实方法签名、穷举所有异常分支，使时序图可直接指导编码。
>
...
1. [步骤 1：谁发起调用，调了谁的什么方法，目的是什么]
2. [步骤 2：被调用方做了什么处理，又调用了谁，传了什么参数]
3. [步骤 3：成功时结果如何逐层返回，状态如何更新]
4. [步骤 4：失败时异常如何传播，错误转换/重试/降级的具体逻辑]
