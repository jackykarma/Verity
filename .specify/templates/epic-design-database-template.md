...
> **定位**：本文件对应 `epic-design.md` **§十一**，面向方案评审，完整描述本 EPIC 涉及的所有数据库表结构设计，与各 Feature 的 `plan.md` §三「数据存储约束」对齐，确保数据模型在评审阶段得到充分论证。
>
> **互指**：`epic-design.md` §十一仅保留定位说明、可选摘要与指向本文件的链接；**正文以本文件为准**。
>
...
- **数据迁移策略**：[版本升级时的 Migration 策略，与 `nfr.md` §9.6 兼容性评估中数据库升级项对应]
- **敏感数据处理**：[加密字段、脱敏策略，与 `nfr.md` §9.5 安全评估对应]

### 11.4 数据库设计一致性自检

- 所有数据表已在对应 Feature 的 `plan.md` §三 中有完整的数据存储规约定义
- §11.1 ER 图中的表关系与 `epic-design.md` §三 领域模型概念一致
- 所有外键关联已在字段约束中明确标注
- 数据库版本迁移策略已在 `nfr.md` §9.6 兼容性评估中覆盖
- 敏感字段已在 `nfr.md` §9.5 安全评估中标注加密/脱敏要求
