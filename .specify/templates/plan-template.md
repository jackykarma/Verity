...
>
> ⚠️ **重要区分**：本节与 A3.2 技术类图的区别
>
> | 维度 | A0.2 领域概念图 | A3.2 技术类图 |
> |------|-----------------|---------------|
...
> - 若 Feature 业务简单（如单一 CRUD），可省略此图，直接进入 A3.2 技术类图
> - 此图中的业务概念（如 `订单`）通常在 A3.2 中会对应多个技术类（如 `OrderEntity`、`OrderDTO`、`OrderUseCase`）
...
| 检查项 | 检查规则 | 如何验证 | 不通过的后果 |
|--------|----------|----------|--------------|
| **A3.0.1 → A3.1** | 框架图中的组件必须 100% 覆盖组件清单 | 对比组件名称（允许简写，但需映射） | 组件边界不清，设计遗漏 |
| **A3.1 → A3.2** | 组件清单中的组件必须在类图中有对应类/接口 | 每个组件至少对应 1 个类/接口 | 设计与实现脱节 |
| **A3.2 → A3.3.1** | 类图中的类必须在时序图中作为参与者出现 | 关键类至少在 1 个时序图中出现 | 静态结构与动态行为脱节 |
| **A3.3.1 → A3.3.2** | 时序图覆盖的流程必须与流程图一一对应 | 每个流程都有时序图+流程图 | 交互路径不完整 |

##### 🔗 图表一致性检查（必须 100% 通过）

| 一致性维度 | 具体规则 | 示例 |
|------------|----------|------|
| **参与者一致性** | 时序图中每个 `participant` 必须在类图中有对应的类/接口 | 时序图的 `UC as UseCase` → 类图的 `class FeatureUseCase` |
| **方法调用一致性** | 时序图中的同步调用（`->>` 实线箭头）必须在类图中有对应的方法 | 时序图的 `UC->>Repo: getData(id)` → 类图的 `+getData(id: String): Flow<T>` |
| **方法签名一致性** | 时序图中的参数和返回值类型必须与类图方法签名完全一致 | 时序图的 `execute(params: RequestParams): Result<T>` 必须与类图定义一致 |
| **错误类型一致性** | 时序图中的错误类型必须在类图的 Error 体系中定义 | 时序图的 `Failure(NetworkError)` → 类图的 `sealed class FeatureError { NetworkError ... }` |
| **数据模型一致性** | 时序图中传递的数据对象必须在类图中有定义 | 时序图的 `Result.Success(DataDTO)` → 类图的 `class DataDTO { ... }` |
| **依赖方向一致性** | 时序图中的调用方向必须与类图的依赖箭头方向一致 | 时序图 `VM->>UC` → 类图 `ViewModel --> UseCase` |

##### 🔍 详细验证步骤

**步骤 1：参与者 100% 覆盖检查**
```
时序图参与者列表（从 A3.3.1 提取）：
- UI/ViewModel
- UseCase
- Repository
- DataSource
...
**步骤 2：方法调用 100% 覆盖检查**
```
时序图中的关键调用（从 A3.3.1 提取）：
1. execute(params: RequestParams)
2. getData(id: String)
3. fetch(id: String)
4. toEntity(dto: DataDTO)

类图中的方法（从 A3.2 提取）：
- ✅ FeatureUseCase.execute(params: RequestParams): Result<T>
- ✅ FeatureRepository.getData(id: String): Flow<T>
- ✅ RemoteDataSource.fetch(id: String): Result<DTO>
- ✅ EntityMapper.toEntity(dto: DataDTO): DataEntity
...
| 问题类型 | 症状 | 修复方式 |
|---------|------|----------|
| 时序图出现未定义的类 | 参与者在类图中找不到 | 在类图中补充该类定义 |
| 方法签名不匹配 | 时序图的参数/返回值与类图不一致 | 统一为类图的签名（单一事实源） |
| 错误类型未定义 | 时序图抛出的异常在 Error 体系中不存在 | 在类图的 FeatureError 中补充 |
| 调用方向反了 | 时序图 A→B，但类图 B→A | 检查依赖方向，修正为正确的分层依赖 |
| 缺少数据模型定义 | 时序图传递 DTO 但类图没画 | 在类图中补充 DTO 类定义 |

##### ✅ 自检清单（逐条勾选）

- [ ] `A3.0.1` 框架图中的组件 **100% 覆盖** `A3.1` 的组件清单（同名或可追溯映射）
- [ ] `A3.1` 组件清单中的每个组件在 `A3.2` 类图中至少有 1 个对应类/接口
- [ ] `A3.2` 全局类图 **100% 覆盖**：UI / ViewModel / UseCase / Repository / DataSource / Entity / DTO / Error / Mapper
- [ ] `A3.2` 类图中的所有公共方法都写了**完整签名**（参数类型 + 返回值类型）
- [ ] `A3.3.1` 时序图中的所有 `participant` 在 `A3.2` 类图中都有对应类/接口
- [ ] `A3.3.1` 时序图中的所有同步调用在 `A3.2` 类图中都有对应方法
- [ ] `A3.3.1` 时序图中的所有错误类型在 `A3.2` 类图的 Error 体系中都有定义
- [ ] `A3.3.1` 时序图集 **100% 覆盖** `A3.3.2` 流程图的每个流程（且同图含异常）
- [ ] `A3.3.1` 时序图中的每个异常分支都映射到 `A3.3.2` 流程图的异常清单（EX-xxx）
...
#### A3.2.2 类图粒度控制（必须遵守：避免过度设计）

> **目的**：明确类图应该包含什么、不应该包含什么，确保粒度适中。

##### ✅ 类图中应该包含的内容

| 内容类型 | 说明 | 示例 |
|---------|------|------|
| **公共接口/抽象类** | 组件间的契约定义 | `interface FeatureRepository { ... }` |
| **核心实现类** | 实现契约的关键类 | `class FeatureRepositoryImpl { ... }` |
| **领域模型/数据模型** | Entity、DTO、VO 等数据结构 | `class DataEntity { +id: String }` |
| **错误类型体系** | Sealed Class 或错误码枚举 | `sealed class FeatureError { ... }` |
| **关键工具类** | Mapper、Converter、Validator 等 | `interface EntityMapper { ... }` |
| **公共方法签名** | 对外暴露的 public 方法 | `+execute(params: RequestParams): Result<T>` |
| **关键成员变量** | 影响类职责和状态的字段 | `+state: StateFlow<UiState>` |
| **依赖关系** | 类/接口之间的依赖箭头 | `ViewModel --> UseCase : uses` |

##### ❌ 类图中不应该包含的内容（过度设计）

| 内容类型 | 为什么不应该包含 | 反例 |
|---------|------------------|------|
| **私有辅助方法** | 实现细节，非契约部分 | ❌ `-validateInput()` |
| **自动生成的 getter/setter** | Kotlin data class 默认提供 | ❌ `+getId(): String` |
| **局部变量/临时变量** | 方法内部实现细节 | ❌ `tempResult: String` |
| **第三方库的类** | 除非自定义扩展，否则无需画出 | ❌ `class OkHttpClient { ... }` |
| **Android SDK 基类** | 除非覆写了关键方法 | ❌ `class ViewModel { ... }` |
| **日志/调试代码** | 非核心业务逻辑 | ❌ `+log(message: String)` |
| **构造函数参数** | 除非有特殊依赖注入逻辑 | ❌ `FeatureViewModel(useCase)` |
| **内部类（非关键）** | 仅用于内部实现的辅助类 | ❌ `inner class Helper { ... }` |

##### 边界案例判断规则

| 场景 | 是否包含 | 理由 |
|------|----------|------|
| ViewModel 的 `private fun updateState()` | ❌ 不包含 | 内部实现，非公共契约 |
| Repository 的 `private fun mergeCache()` | ❌ 不包含 | 内部策略，除非复杂到需要单独说明 |
| UseCase 的 `operator fun invoke()` | ✅ 包含 | Kotlin 特殊方法，是公共契约 |
| 自定义的 `ValidationException` | ✅ 包含 | 错误体系的一部分 |
| Retrofit 的 `@GET` 注解方法 | ✅ 包含 | 定义了网络契约 |
| Room 的 `@Query` 注解方法 | ✅ 包含 | 定义了数据库契约 |
...
##### 方法签名对照表（必须：与类图 A3.2 保持一致）

> **目的**：明确时序图中每个调用的完整签名，确保与类图中的方法定义一致。

| 参与者 | 方法调用 | 完整签名 | 返回值说明 |
|--------|----------|----------|------------|
| UI → UseCase | `execute(params)` | `execute(params: RequestParams): Result<Data, FeatureError>` | 成功返回 Data，失败返回 FeatureError |
| UseCase → Repository | `getData(id)` | `getData(id: String): Flow<DataEntity>` | 返回数据流（支持缓存+实时更新） |
...
**一致性检查**：
- [ ] 所有方法调用在类图（A3.2）中都有对应定义
- [ ] 参数类型和返回值类型与类图完全一致
- [ ] 错误类型在 FeatureError（A3.2）中有定义
...
|---------|------|----------|------------|----------|----------|
| **Feature 全局设计** | A3.2 | 所有关键类的公共接口（签名级别） | 端到端主流程（跨组件交互） | **必须产出** | UI/ViewModel/UseCase/Repository/DataSource/Entity/DTO/Error/Mapper 的完整静态结构 |
| **组件详细设计** | A3.4 | 组件内部所有类（含 private 类） | 组件内部交互（方法间调用） | Plan Level = Deep 时产出 | 复杂组件（如状态机、策略模式）的内部实现 |
...
**类图包含**：
- ✅ 仅涉及本 Story 的类（从 A3.2 中提取）
- ✅ 可细化到 private 方法（如有必要）
- ❌ 不得新增 A3.2 未定义的核心类/接口
...
开始
  ↓
是否是新 Feature？
  ├─ 是 → 必须产出 A3.2 Feature 全局类图 + A3.3 时序图集
  └─ 否 → 跳过
  ↓
...
Plan Level 是否为 Deep？
  ├─ 是 → 产出 C1 Story 详细设计
  └─ 否 → 在 A3.2/A3.3 基础上直接进入 tasks.md
```
...
| 错误做法 | 为什么错 | 正确做法 |
|---------|---------|---------|
| 在 Story 设计中新增核心类 | 违反组件边界，影响全局设计 | 回到 A3.2 修订全局类图，提升 Plan Version |
...
| Feature 类图缺少 Mapper | 数据转换逻辑散落在 Repository 中 | 在 A3.2 中补充 EntityMapper 类 |
