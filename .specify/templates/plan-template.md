...
> - **第二层（Feature 全景）**：全景类图 + Feature 级时序图/流程图 + 关键设计详解（若适用）
...
> **适用条件**：Feature 中存在 **1 个及以上** 关键、复杂或高风险的 design point，需要专项展开说明时使用。
>
> **与 A3.1.2.3 的关系**：A3.1.2.3 记录「决策摘要」（一行一决策）；本节对需要深入说明的设计点做**详细展开**。两者通过决策点名称或 ID 互引。
>
> **何时省略**：若所有设计点用 A3.1.2.3 表格 + A3.2 全景图即可表达清楚，可省略本节。

###### 设计点 1：[设计点名称]

- **背景**：[为何是关键/难点，涉及哪些组件或跨层关注点]
- **关联决策**：A3.1.2.3 决策表 → [决策点名称]

**详细设计**（按需选用：类图 / 时序图 / 流程图 / 状态机 / 伪代码）

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#E3F2FD', 'primaryTextColor': '#1565C0', 'primaryBorderColor': '#1976D2', 'lineColor': '#546E7A'}}}%%
flowchart TD
    Placeholder[此处放置状态机/流程图/类图等]
```
...
- [ ] `A3.2.4`（若启用）每个关键设计点与 A3.1.2.3 决策表有对应引用
...
> 目的：把"枚举"落到"可执行对策"，并与 A3.2.2 / A3.2.3 / A3.2.4（若启用）/ A3.3 的异常分支互校。
>
> **对策类型说明**：
