...
>   - **Deep**：新契约/持久化迁移/复杂动效与性能预算/并发竞态/灰度回滚等 — 在 Standard 基础上，补齐**所有** Story 的 L2 详细设计（按序号覆盖，不得遗漏）
>
...
> - Plan Level = Deep：在 Standard 基础上，还需要补充**所有** Story 的 L2 详细设计（按 Story 序号覆盖，不得遗漏；见后文 Story Detailed Design）
>
...
Story Breakdown 从**技术视角**拆分 Feature，服务于三类目标：
...
- **工作量估算基准**：按**普通程序员**水平评估，需包含完整开发周期——编码、联调、Code Review、用例自检（含单元测试/集成测试）等环节，不得仅按纯编码时间估算。
...
| 类型 | 适用场景 | 典型特征 | 验收侧重 |
|------|----------|----------|----------|
| **Infrastructure** | 数据库、表结构、迁移脚本、基础框架搭建 | 无业务逻辑、下游依赖 | 结构正确、可执行、无破坏 |
| **Design-Enabler** | 数据访问接口、能力抽象、供下游消费的契约 | 定义接口/契约，实现可后补 | 契约完整、可 Mock、可替换 |
| **Functional** | 主流程、业务逻辑、UI 交互、用户可见能力 | 直接对应 FR、可端到端验证 | 行为符合 FR、AC 可测 |
| **Optimization** | 性能、内存、功耗、体验优化 | 在已有功能基础上做增强 | 指标达标、无回归 |
...
**并行与冲突避免**（服务于多人协作与独立提交）：

| 原则 | 实操要点 |
|------|----------|
| **接口契约前置** | 数据/能力 Story 完成时，下游消费方所需的接口、错误码、数据结构已稳定；ViewModel/UI Story 依赖「接口」而非「实现细节」 |
| **改动范围不重叠** | 同层内可按模块/屏幕拆分（如 ST-003a 列表页、ST-003b 详情页），避免多人改同一文件 |
| **可并行标注** | 无依赖关系的 Story 必须标注「可并行：是」，并说明各自改动路径；有依赖的标注「否」并写明依赖链 |
| **最小提交单元** | 每个 Story 对应至少一次可独立合并的 PR；不把「数据库 + ViewModel + UI」打包成一个 Story |

### 拆分决策流程

1. **是否含 UI + 数据 + 业务**？→ 是：优先采用五类 Story；否：参考下方「其他拆分策略」。
2. **单个 Story 是否 > 10 人天**？→ 是：按层/模块/风险/场景继续拆分；否：保持。
3. **是否有独立能力/算法**？→ 是：每个能力单独一个 Story；否：跳过。
4. **是否有技术风险或第三方依赖**？→ 是：考虑增加预研/选型 Story；否：跳过。
5. **依赖关系是否清晰且无环**？→ 否：重新划分边界；是：通过。
...
| 反模式 | 问题 | 正确做法 |
|--------|------|----------|
| **上帝 Story** | 一个 Story 包含建表 + 数据访问 + ViewModel + UI，难以并行、提交粒度粗 | 严格按五类拆分，每类边界清晰 |
| **循环依赖** | ST-001 依赖 ST-002，ST-002 又依赖 ST-001 | 调整边界，确保 DAG；必要时引入中间抽象 |
| **无消费方的 Enabler** | Design-Enabler/Infrastructure Story 无下游依赖，变成孤岛 | 每个 Enabler 必须有明确的消费 Story，或合并到有消费方的 Story |
| **粒度过细** | Story < 2 人天且无法独立验收，仅为「写一个类」 | 合并到相邻 Story，或按「可独立验证」重新划分 |
| **命名模糊** | 标题含「和」「或」「以及」过多，职责不清 | 用单一动词+名词，如「实现列表页 UI」而非「实现列表和详情及错误处理」 |
| **跨组件 Story** | Story 横跨 A3.1.2.1 中多个组件，改动范围过大 | Story 必须在单一组件边界内，跨组件协作通过接口 |
| **忽视现有代码** | 按理想架构拆分，忽略既有模块与包结构 | 遵循章程「演进式设计」：在现有组件边界内拆分，不新增未决策的组件 |
...
- [ ] 可并行的 Story 改动路径不重叠？（避免同一文件被多人同时修改）
...
- **描述**：[做什么、为什么]
- **目标**：[可验证的结果]
- **预估工作量**：[X 人天]（必填，≤ 10 人天；含编码、联调、CR、自检，按普通程序员评估）
- **覆盖 FR/NFR**：FR-???；NFR-???
- **依赖**：[其他 Story / 外部依赖 / 无]
- **可并行**：是/否（原因）
- **关键风险**：是/否（关联 RISK-???）
...
> 说明：
> - **估算基准**：按普通程序员水平，含编码、联调、Code Review、用例自检（含单元/集成测试）时间
...
> 
> **覆盖范围**：执行 Deep 阶段输出时，必须按 Story 序号（ST-001、ST-002、…）覆盖 Story Breakdown 中的**所有** Story，不得遗漏。
> 
> 目标：在 L0（全景边界）与 L1（组件/协作）方案评审通过后，对所有 Story 补充"落码方式"细节，做到**不写每行代码**也能明确指导开发如何落地。
> 建议顺序：先完成 Plan-A 的 A2/A3（0 层/1 层设计）→ 再按 Story 序号依次补齐所有 Story 的本节（L2）。
>
> 规则：
> - 本节内容属于 Plan 的一部分，视为**权威技术决策输入**（必须纳入版本管理与变更记录）。
> - 各 Story 的 L2 详细设计写在独立文档 **[story_detail_design.md](./story_detail_design.md)** 中；必须与 plan.md 同目录放置；模板见 `.specify/templates/story_detail_design_template.md`。
...
>
> **内容完整性约束**（必须遵守，Review 可据此驳回）：
> - **禁止省略或引用**：需求描述、触发条件、系统响应须在 L2 文档内完整书写，不得使用「见 A3」「同上」「参见 xxx」等省略或引用方式。
> - **类图必须完整**：每个 Story 须在 L2 中绘制**完整的**类图，覆盖本 Story 涉及的全部关键类与方法签名；**不得引用** plan.md 组件设计（A3）的类图。
> - **时序图必须完整**：每个 Story 须在 L2 中绘制**完整的**时序图，覆盖正常流程与关键异常分支；**不得引用** plan.md 组件设计的时序图。
>
> **L1→L2 覆盖约束**（必须遵守，详见 `docs/speckit/story-l2-l1-coverage.md`）：
> - **类覆盖**：所有 Story 的 L2 类图合起来必须覆盖 A3.2.2 全景类图的**全部**类；每个 Story 须填写「本 Story 覆盖的 L1 类」。
> - **粒度约束**：L2 类图的方法/成员粒度不得低于 L1，应更细化（含参数类型、返回类型等）。
> - **覆盖矩阵**：story_detail_design.md 末尾必须填写「L1 类覆盖矩阵」，追溯每个 L1 类由哪些 Story 覆盖。
>
> **硬约束（Story 级设计边界）**：
> - **Story 级设计不得新增组件级概念**。Story Detailed Design 只能在 A3.1.2.1/A3.3 已定义的组件边界内做细化，不得：新增组件、新增核心类/接口、新增错误类型。
> - **若发现需要新增上述内容**，必须回到 A3 修订、提升 Plan Version、在变更记录中注明影响范围。
> - **Review 否决依据**：任何违反上述约束的 Story 设计输出，应被驳回并要求修订。
>
