...
if (-not (Test-Path $paths.FEATURE_DIR -PathType Container)) {
    ...
    Write-Output "Run /aisdd.feature first to create the Feature directory and spec.md."
    exit 1
}

if (-not (Test-Path $paths.IMPL_PLAN -PathType Leaf)) {
    Write-Output "ERROR: plan.md not found in $($paths.FEATURE_DIR)"
    Write-Output "Run /aisdd.plan first to create the implementation plan."
    exit 1
}
...
if ($RequireTasks -and -not (Test-Path $paths.TASKS -PathType Leaf)) {
    ...
    Write-Output "Run /aisdd.tasks first to create the task list."
    exit 1
}
